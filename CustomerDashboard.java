...
@FXML private Button store_btn, history_btn, account_btn, enquiry_btn;
    ...
    public void initialize() {
        ...
        getItems(); // Presumably this populates productNameChoice and productSizeChoice
        setAccountPane();

        username.setText(Account.getUser());

        // Safely set default values only if items exist
        if (!productNameChoice.getItems().isEmpty()) {
            // Safe to set index 0 if at least one item exists
            int index = Math.min(1, productNameChoice.getItems().size() - 1);
            productNameChoice.setValue(productNameChoice.getItems().get(index));
        }

        if (!productSizeChoice.getItems().isEmpty()) {
            int index = Math.min(2, productSizeChoice.getItems().size() - 1);
            productSizeChoice.setValue(productSizeChoice.getItems().get(index));
        }
